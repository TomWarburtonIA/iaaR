...
mcm_rates <- function(species_name) {
  ...
  #Search for complex reactions
  contains_complex <- any(grepl("complex rate, refer to MCM", reactions$Rate))
  
  #If complex reactions found, remove and return
  if(contains_complex==TRUE) {
    warning("Complex rates found, removing from returned data frame.")
    reactions <- reactions[!grepl("complex", reactions$Rate), ]
    return(reactions)
  } else {
    return(reactions)
  }
